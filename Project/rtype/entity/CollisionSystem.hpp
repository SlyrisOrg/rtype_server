...
#include <rtype/entity/QuadTree.hpp>
...
namespace rtype
{
    ...
    {
    public:
        explicit CollisionSystem(EntityManager &ettMgr,
                                 QuadTree <EntityManager> &quadtree,
                                 lua::LuaManager &lua) noexcept : _ettMgr(ettMgr), _lua(lua), _quadtree(quadtree)
        {
        }
            ...
            for (auto &&node : _nodes) {
                ...
                for (size_t i = 0; i < size - 1; ++i) {
                    ...
                    for (auto &&item : itemsQ) {
                        const auto &itemBox = _ettMgr[item].getComponent<rtc::BoundingBox>();
                        const auto &curItemBox = _ettMgr[curItem].getComponent<rtc::BoundingBox>();
                        if (curItemBox.AABB.intersects(itemBox.AABB)) {
