...
function internal_collide(meID, collideID)
    local meEtt = getEntity(meID)
    local collideEtt = getEntity(collideID)
    if meEtt:hasPlayerComponent() and collideEtt:hasPlayerComponent() then
        return
    end
    speed = -1500;
    forced_move = true;
...
function internal_setPosition(meID, x, y)
    getEntity(meID):getBoundingBoxComponent().setPosition(x, y);
end
...
playerTable = {
    ...
    setPostion = internal_setPosition;
}